--[[
    ...
    >> I take no credit for those parts of this file, some modifications have been made but its mostly their work and should work mostly the same
    >> but only the Intended async/await function this exports and the AsyncTask Object have been tested.
]]
...
function AsyncTasks:RunTasks()
    for idx, task in ipairs(self.Tasks) do
        if (type(task) == 'table') and (type(task.thread) == 'function') then
            ...
            if (not taskOk) then
                (Log or print)(string.format('AsyncTask Error Task[%s] %s > %s', tostring(idx), tostring(task.name), tostring(taskResult)))
            end
        end
    end
end
...
function AsyncTasks:SchedualUpdateThread(updateDelay)
    ...
    if thread_Run then -- is thread enabled
        ...
        self.TimerId = Script.SetTimerForFunction((updateDelay * 1000), 'asyncTasks_thread', self)
    end
end

--- Starts this Thread, if thread allready started does nothing
function AsyncTasks:Start()
...
asyncTasks_thread = function(self)
    task_schedule_all()
...
AsyncTasks.task_schedule_all = task_schedule_all
AsyncTasks.task_schedule = task_schedule
AsyncTasks.task_yield = task_yield

RegisterModule("mFramework2.Modules.AsyncTasks",AsyncTasks)
return AsyncTasks