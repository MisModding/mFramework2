    ...
    Client = {
        ClSetCVarString = function(self, varName, valueString)
            ...
            mFramework2.Debug('CustomPlayer',
                              string.expand('Successfully SetCVar: ${var} newvalue: ${val}', {var = varName, val = valueString}))
        ...
        ClSetCVarNumber = function(self, varName, valueNumber)
            ...
            mFramework2.Debug('CustomPlayer', string.expand('Successfully SetCVar: ${var} newvalue: ${val}',
        ...
        ClEventReceive = function(self, event_string, source_Id, target_Id)
            ...
            mFramework2.Debug('CustomPlayer',
                              string.expand('Recieved Event: ${name} data: ${data}', {name = event.type, data = event_string}))
...
function CustomPlayer:SendMultipartEvent(To,event,sourceId,targetId,isBroadcast)
    ...
    for idx,chunk in ipairs(event) do
        if To == "server" then
            self:SendEventToServer({ Type = "xMultipart", Data = {on = "server", chunk = idx , chunks = chunks, data = chunk}},sourceId,targetId)
        else
            self:SendEventToClient({ Type = "xMultipart", Data = {on = "client", chunk = idx , chunks = chunks, data = chunk}},sourceId,targetId,isBroadcast)
        end
    end
end
...
function CustomPlayer:CustomEventHandler(event, source_Id, target_Id)
    ...
    if event.type == "xMultipart" then
        mFramework2.Debug('CustomPlayer:MultipartEvent', string.expand('Processing Data: ${chunk}/${chunks}', {chunk = event.data.chunk, chunks = event.data.chunks}))
        ...
        return RecieveMultipart(self, {id = 1,chunk = event.data['chunk'], chunks = event.data['chunks']}, event.data['data'],handleEvent)
    end
    -- try to find Handler
