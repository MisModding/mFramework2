...
local db_funcs = {
    save = function(db, p)
        ...
        for p, page in pairs(db) do if not store_page(pool[db] .. '/' .. p, page) then return false end end
        return true
    end,
}
local mt = {
...
function collection:GetPage(pageId)
    ...
    if (data == nil) or (data == json.null) then return false, 'no page data for pageId:' .. pageId end
...
---@field new fun(self:MisDB.DataStore,config:table):MisDB.DataStore
local DataStore = {}
...
function DataStore:new(config)
    ...
    self.DataSource = {
        Source = MisDB(config.persistance_dir), ---@type MisDB
    }
    self.DataSource['Data'] = self.DataSource['Source']:Collection(config.name) ---@type MisDB.Collection
    return self
end
